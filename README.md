...
- 支持火山方舟和其他 OpenAI 兼容的 Chat Completions 服务。
...
- 服务：`火山方舟`
- 接入方式：`方舟 API` 或 `Coding Plan`
- 方舟 API Endpoint: `https://ark.cn-beijing.volces.com/api/v3`
- Coding Plan Endpoint: `https://ark.cn-beijing.volces.com/api/coding/v3`
...
- `其他 OpenAI 兼容服务`，例如 DeepSeek、OpenRouter、Moonshot、硅基流动等支持 `/chat/completions` 的服务

API Key 会保存在本机 macOS Keychain 中，不会写入代码仓库。
